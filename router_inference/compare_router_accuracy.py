...
def build_complete_evaluation_dictionary() -> Dict[str, Dict[str, Tuple[float, float]]]:
    ...
    for file_name in result_files:
        ...
        for global_index, result in extracted_results.items():
            if "evaluation_result" in result and result["evaluation_result"] is not None:
        ...
        print(f"  Added {len(extracted_results)} results for {model_name} (unvalidated: {unvalidated_count})")
...
def load_predictions(router_name: str, config: Dict):
    ...
    if router_name == 'vllm':
        ...
        for pred in predictions:
            pred["all_confidence"] = {model_name_manager.get_universal_name(pred["prediction"]): 1.0}
        ...
        if confidence_field and prediction[confidence_field]:
            ...
            for i, model_name in enumerate(models_list):
                if i < len(prediction[confidence_field]) and costs[i] is not None:
                    universal_name = model_name_manager.get_universal_name_non_static(model_name)
                    if isinstance(prediction[confidence_field], dict):
                        confidence_dict[universal_name] = list(prediction[confidence_field].values())[i]
                    else:
                        confidence_dict[universal_name] = prediction[confidence_field][i]
...
def main():
    ...
    routerevalbench = load_dataset("arrow", data_files=f"{dataset_path}/data-00000-of-00001.arrow")
    ...
    for router_name in router_names:
        ...
        for prediction in predictions:
            global_index = prediction.get('global_index', prediction.get('global index', ''))
            all_confidence = prediction.get('all_confidence', prediction.get('all confidence', {}))
            ...
            assert model_selected in evaluation_dict, f"Model {model_selected} not found in evaluation dictionary"
            ...
            try:
                ...
                print(f"Router {router_name}: Model {model_selected} not found in evaluation dictionary for global index {global_index}")
