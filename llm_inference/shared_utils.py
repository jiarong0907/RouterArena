...
def log_pipeline_start(logger, pipeline_type: str, model_name: str, start_time: datetime.datetime, seed: int):
...
def log_pipeline_completion(logger, pipeline_type: str, model_name: str, start_time: datetime.datetime, 
                          end_time: datetime.datetime, results: list = None):
    ...
    if results:
        ...
        logger.info(f"Total tokens: {total_tokens:,} | Avg per success: {total_tokens / len(successful_results):.1f}" if successful_results else "No successful results")
    
    logger.info("="*60)

def create_result_entry(global_index: int, question: str, model_name: str, 
                       inference_result: dict, additional_fields: dict = None) -> dict:
